...
@section("content")
    ...
    <div class="card">
        ...
        <div class="card-body">
            <h1>Gelen Portakallı Lezzetler Mailleri Formları</h1>
            <div class="inbox">
            <div class="mail-list">
                <ul>
                    ...
                    @foreach($gelenMailler as $mail)
                        <li class="@if($mail->markRead == 0) unread @endif" id="mail{{$mail->id}}">
                            <a href="javascript:void(0)" onclick="getForm({{$mail->id}})">
                                <div class="img">{{\Illuminate\Support\Str::limit($mail->ad,1,'')}}</div>
                                <div class="gonderen-bilgi">
                                    <div class="gonderen">{{\Illuminate\Support\Str::limit($mail->ad,20,'...')}}</div>
                                    <div class="aciklama">{{\Illuminate\Support\Str::limit($mail->yemekadi,20,'...')}}</div>
                                </div>
                            </a>
                        </li>
                    @endforeach
        ...
        @section('extraJs')
            ...
            <script>
                function getForm(mailId) {
                    axios.get(`/yonetim/forms/get-pmail?id=${mailId}`)
                        .then(response => {
