...
@section("content")
    <div class="card">
        {{-- card-header kısmını şöyle yapabilirsin: --}}
        <div class="card-header">
            @if(isset($selectedPage))
                <span class="text-warning">{{ $selectedPage->title }}</span> Sayfasına Ait Özellikler
                {{-- İstenirse tümüne dönmek için bir buton konulabilir --}}
                <a href="{{ route('cms.features.index') }}" class="btn btn-sm btn-primary float-end">Tümünü Göster</a>
            @else
                Tüm Sayfa Özellikleri Listesi
            @endif
        </div>
        <div class="card-body">
            <table id="datatable" class="display stripe table-responsive-sm table-responsive-md"
                ...
                @foreach($features as $item)
                    <tr>
                        ...
                        <th class="islemler">
                            <a href="{{ route("cms.features.edit",$item->id) }}"
                               class="btn bg-primary" title="Düzenle">
                                <i class="las la-pen"></i>
                            </a>
                            <a onclick="deleteFunc('{{ route("cms.features.destroy",$item->id) }}')"
