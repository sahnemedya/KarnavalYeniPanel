...
@section("content")
    <div class="card">
        <div class="card-header">Silinen Referans Türleri</div>
        <div class="card-body">
            <table id="datatable" class="display stripe table-responsive-sm table-responsive-md"
                ...
                @foreach($referenceTypes as $item)
                    <tr data-id="{{ $item->id }}">
                        ...
                        <th class="islemler">
                            <a onclick="restore('{{route("cms.reference-types.restore",$item->id)}}')"
                               class="btn bg-warning" title="Geri Yükle">
                                <i class="las la-recycle"></i>
                            </a>
                            <a onclick="deleteFunc('{{route("cms.reference-types.forceDelete",$item->id)}}')"
